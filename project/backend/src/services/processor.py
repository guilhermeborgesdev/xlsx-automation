...
def processar_planilha(uploaded_file, usuario):
    def formatar_data(data_str):
        ...
        for fmt in formatos:
            try:
                return datetime.strptime(data_limpa, fmt).strftime("%d/%m/%Y %H:%M:%S")
            except:
                continue
        return ""
    ...
    final_df = pd.DataFrame(columns=['A', 'B', 'C', 'D'])
    for _, row in df.iterrows():
        ...
        if data and lat and lon:
            final_df.loc[len(final_df)] = [placa, data, lat, lon]

    registrar_edicao_planilha(usuario, uploaded_file.name)
    adicionar_ao_historico(usuario, f"Editou a planilha {uploaded_file.name}")
        ...
        if st.button("Processar Arquivos"):
            ...
            for i, uploaded_file in enumerate(uploaded_files):
                try:
                    ...
                    novo_df, extensao, linhas_validas = processar_planilha(uploaded_file, usuario)
                    if novo_df is None:
                        continue
                    output = io.StringIO()
